      ...
      {isSmallScreen ? (
        <div className="">
          {Array.from({ length: totalPages }).map((_, pageIndex) => {
            ...
            // Check if startIndex is within the bounds of ComicCategories
            if (startIndex >= totalImages) return null; // Prevent rendering if out of bounds

            // Get the images for the current page
            const imagesForPage = getImagesForPage(
              startIndex,
              Math.min(namesToShow, totalImages - startIndex) // Ensure we don't go past the length
            );

            // Choose the correct PageComponent based on how many names to show
            const PageComponent = {
              1: NewHomePageContent,
              7: Page6,
              8: Page5,
            }[namesToShow];

            // Only render the page if there are images for it
            return (
